...
1. **前端**：`dashboard/` 为 Vite + React 19 + TypeScript SPA，
   Tailwind 3 + `@yoi/design` preset（与 `web/` 同 major，preset 直接
   复用），组件用 shadcn（CLI 安装，不手写原语），图标用 lucide-react
...
- **Tailwind 4**：`@yoi/design` preset 是 Tailwind 3 格式（`require` +
  插件），为与 `web/` 保持一致、零迁移成本，钉在 3.4。未来两边一起升级。
...
- mock 数据层意味着 PRD 的验收标准可以先对 UI 验收；数据契约的真实
  来源切换是后端 ADR 的主题。
- shadcn 组件经 `@yoi/design` 令牌换肤，与 storefront 观感一致；
...
- 产物 CSS 含 `@yoi/design` 令牌（`--accent` 等），无 shadcn 默认
  灰蓝色板残留。
