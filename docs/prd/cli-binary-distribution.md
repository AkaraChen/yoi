...
- 作为 Agent，fleet 写入需要 `yoi` 时，我向人出示 install 命令，
  得到同意后再执行；不静默安装。
- 作为 Agent，我在 Linux 服务器上用对应脚本装 `yoi-server`，并从同一
  Release 取 `yoi-dashboard`，写好 systemd unit，探针监听
  `127.0.0.1:8788`。
- 作为 Windows 上的人/Agent，我用 PowerShell 脚本装两个 CLI（不是探针）。
- 作为遇到 404 / 缺资产的使用者，install 脚本以明确错误结束，
...
- 两个 CLI 各有独立的 POSIX 与 PowerShell 安装入口（raw.githubusercontent.com）。
...
- 不改 `packs/`，不把 pack `reference/install.sh` 当成产品安装器。
...
1. 人同意安装某个二进制。
2. Agent 运行对应 `curl | sh` 或 `irm | iex`（或等价：先下载再执行）。
3. 脚本探测 OS/架构，从 `releases/latest/download/` 拉档案与
...
- **不支持的 OS/架构**：明确失败。
- CLI 安装脚本是非交互的（`curl | sh` 的 stdin 是脚本本身）。确认
  由 skill / 人在环完成，不在脚本里 `read` yes。
...
- 两个 CLI 各一条 curl 安装路径；探针同 Release 出二进制。
