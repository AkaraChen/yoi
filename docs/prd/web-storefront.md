...
`web/` 是 yoi 的软件源与获客站（pack 目录），不是产品本身。产品是个人
部署栈的自托管 OS（见 `docs/prd/product-positioning.md`）。本站卖的不是
CLI，入口是装进 agent 的 yoi skill（壳 / 路由）+ 每个 pack 内含的产品
skill。CLI 只是 Agent 的系统调用，由 skill 流程按需引导（装前先问）。

获客可以继续讲第一条命令（装 skill、一句话部署、三分钟跑起来）。不得
把本站写成 yoi 的产品定义，也不得把货架品类写成「只做 Agent 产品」。
2026-08-13 的双面结构仍未选型，不在本站体现。

## 目标用户

- 想在自己的 Linux 上跑一套部署栈、准备把操作交给编码 Agent 的人。
- 被分享链接带到某个 pack 详情页的潜在用户（发现软件源里的某一项）。
...
- **叙事 = 获客可以讲第一条命令**（装 skill、一句话部署），产品定义是
  OS 不是商店（2026-09-01，见 `docs/prd/product-positioning.md`）。货架
  品类开放；现有 Agent 向 pack 不是品类承诺。双面结构仍未选型。
  本 PRD 不改首页已上线文案（「网红产品」「三分钟」等）；改商店文案需
  另一次质问。
