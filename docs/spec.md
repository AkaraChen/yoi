...
- A service detail shows five blocks: status (the service's `desired_state` — intent, not live state), spec declaration (ports and resource limits from the spec JSON), release history (expandable plan/config/outcome), event timeline, and external links (from the spec's `links` array; absent links mean no links block).
- The dashboard styles through `@yoi/design` semantic tokens only; it consumes data exclusively through a JSON API boundary (`src/lib/api.ts` in the SPA). Service endpoints are real: the probe reads the server-world `~/.yoi` store (written by `yoi-server`) through a small read-only store-reader package (`dashboard/server/store/`) that parses the markdown frontmatter/section and NDJSON conventions; the store path resolves as `-store` flag → `YOI_DASHBOARD_STORE` → `~/.yoi`, and a missing store yields empty states, not errors.
...
- Live host metrics (CPU/memory/disk/network/load) are served at request time by the Dashboard Go probe and are never persisted. Service-level live occupancy (container/process CPU and memory) is deferred; the service detail shows the spec declaration instead. Resource facts are never written to the store.
