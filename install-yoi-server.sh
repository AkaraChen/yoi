#!/bin/sh
# Install the server-world `yoi-server` CLI from the latest GitHub Release.
# Usage: curl -fsSL https://raw.githubusercontent.com/AkaraChen/yoi/main/install-yoi-server.sh | sh
set -eu
...
Darwin) os=darwin ;;
MINGW*|MSYS*|CYGWIN*)
	die "Windows: use install-yoi-server.ps1 (irm https://raw.githubusercontent.com/AkaraChen/yoi/main/install-yoi-server.ps1 | iex)"
	;;
...
tmp="${TMPDIR:-/tmp}/yoi-server-install.$$"
mkdir -p "$tmp"
